...
export function useSources() {
  ...
  const addSource = useCallback(
    async (data: SourceFormData): Promise<void> => {
      try {
        await addSourceToDb({
          name: data.name,
          url: data.url,
  ...
  const updateSource = useCallback(
    async (id: string, updates: Partial<SourceFormData>): Promise<void> => {
      try {
  ...
  const syncSource = useCallback(
    async (id: string): Promise<void> => {
      try {
        ...
        await refreshSources()
        throw new Error(
          err instanceof Error ? err.message : 'Failed to sync source'
        )
      }
    },
    [refreshSources]
  ...
  const syncAllSources = useCallback(async (): Promise<void> => {
    try {
      ...
      await refreshSources()
      throw new Error(
        err instanceof Error ? err.message : 'Failed to sync sources'
      )
    }
  }, [refreshSources])
  ...
  const exportSources = useCallback(async (): Promise<SourcesExport> => {
    try {
      ...
      throw new Error(
        err instanceof Error ? err.message : 'Failed to export sources'
      )
    }
  }, [sources])
